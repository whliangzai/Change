...
class RecordingClient:
    ...
    def fetch_dataset(self, dataset, codes=None, **kwargs):
        ...
        if dataset == "security_master":
            rows = []
            for code in codes:
                rows.append({"thscode": code, "listedDate": "2010-01-01", "delistedDate": None, "board": "MAIN"})
            return rows
        if dataset == "daily_bars":
            return [{"thscode": code, "tradeDate": trade_date.isoformat(), "open": 10, "high": 11, "low": 9, "close": 10.5, "volume": 100, "amount": 1000} for code in codes]
        if dataset == "adjustment_factors":
            return [] if not self.complete else [{"thscode": code, "tradeDate": trade_date.isoformat(), "af": 1} for code in codes]
        if dataset == "historical_status":
            return [{"thscode": code, "tradeDate": trade_date.isoformat(), "isST": False, "isSuspended": False, "isDelistingArrange": False} for code in codes]
        if dataset == "industry_membership":
            return [{"thscode": code, "industryCode": "C20", "inDate": "2010-01-01", "outDate": None} for code in codes]
        return []


def _service(tmp_path: Path, *, complete: bool = True):
    engine = create_engine(f"sqlite:///{tmp_path / 'ifind.db'}")
    Base.metadata.create_all(engine)
    repository = SqlAlchemyResearchRepository.from_engine(engine)
    client = RecordingClient(complete=complete)
...
def test_ifind_import_normalizes_and_persists_provenance(tmp_path: Path) -> None:
    ...
    assert result["information_cutoff_at"].endswith("T15:00:00+08:00")
    assert result["available_at"].endswith("T18:30:00+08:00")
