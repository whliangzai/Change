...
def test_repeated_job_key_executes_once_and_replays_result() -> None:
    ...
    first = run_idempotent_task(
        "daily-report", date(2026, 9, 3), work, run_store=store
    )
    second = run_idempotent_task(
        "daily-report", date(2026, 9, 3), work, run_store=store
    )
...
def test_failed_job_keeps_run_log_and_can_be_retried() -> None:
    ...
    recovered = run_idempotent_task(
        "backtest", date(2026, 9, 3), work, run_store=store
    )
