...
def main() -> int:
    ...
    parser.add_argument("--business-date", default=os.getenv("BUSINESS_DATE", date.today().isoformat()))
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()
    if args.dry_run:
        print(f"dry-run: daily report for {args.business_date}")
        return 0
    print("daily command requires an injected application service; manual confirmation remains required")
    return 0
