...
def allocate_quantity(
    *,
    ...
    if account_type.upper() != "CASH" or financing.upper() != "CASH" or leverage != Decimal("1") or allow_short:
    ...
    position_count = len([position for position in positions if position.quantity > 0 and position.symbol != symbol])
    ...
    industry_value = sum((position.market_value for position in positions if position.industry == industry), Decimal("0"))
    remaining_investment = max(Decimal("0"), equity * selected_limits.max_investment_ratio - invested)
    current_value = current_symbol.market_value if current_symbol is not None else Decimal("0")
    remaining_position = max(Decimal("0"), equity * selected_limits.max_position_ratio - current_value)
    remaining_industry = max(Decimal("0"), equity * selected_limits.max_industry_ratio - industry_value)
    notional = min(selected_limits.target_position_value, remaining_position, remaining_industry, remaining_investment, cash)
    ...
    while quantity > 0:
        total = price * Decimal(quantity) + selected_cost_model.calculate(
