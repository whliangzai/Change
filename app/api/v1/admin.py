...
@router.get("/jobs")
def jobs(request: Request, _: Admin, page: int = Query(1, ge=1), page_size: int = Query(50, ge=1, le=200)) -> JSONResponse:
...
def audit_events(
    request: Request, principal: Reviewer, audit_writer: Annotated[object, Depends(get_audit_writer)],
    page: int = Query(1, ge=1), page_size: int = Query(50, ge=1, le=200),
) -> JSONResponse:
    events = getattr(audit_writer, "events", [])
    start = (page - 1) * page_size
    items = [
        {"occurred_at": event.occurred_at.isoformat(), "actor_id": str(event.actor_id) if event.actor_id else None,
         "actor_roles": list(event.actor_roles), "action": event.action, "object_type": event.object_type,
         "object_id": event.object_id, "request_id": event.request_id, "result": event.result}
        for event in events[start : start + page_size]
    ]
    return _success(request, {"items": items, "page": page, "page_size": page_size, "total": len(events)})
